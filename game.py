...
class Game:
  def __init__(self, bird_img, pipe_img, background_img, floor_img):
    ...
    self.rotated_bird = pygame.transform.rotozoom(self.bird,-self.bird_movement * 3, 1)
    self.font = None
    
  def resize_images(self):
    self.bird = pygame.transform.scale(self.bird, (51, 34))
    self.pipe = pygame.transform.scale(self.pipe, (80, 438))
    self.floor = pygame.transform.scale(self.floor, (470, 160))
  ...
  def draw_floor(self, screen):
    screen.blit(self.floor, (self.floor_position, 650))
    screen.blit(self.floor, (self.floor_position + 470, 650))
    
  def move_floor(self):
    self.floor_position -= 1
    if self.floor_position <= -self.width:
      self.floor_position = 0
